...
def mirror_field(b0=B0, alpha=ALPHA):
    '''Axisymmetric magnetic mirror.'''

    def B_field(x):
        x1, x2, x3 = x
        r_sq = x1 * x1 + x2 * x2
        b_axial = b0 * (1.0 + alpha * (x3 * x3 - 0.5 * r_sq))
        b_radial = -b0 * alpha * x3
        return np.array([b_radial * x1, b_radial * x2, b_axial])
    return StaticField(B_func=B_field)
