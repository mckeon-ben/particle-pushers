...
DATA_DIR = 'data'
DATA_FILE = os.path.join(DATA_DIR, 'harmonic_well.json')

# Names the figure.
EXPERIMENT = 'Harmonic well'
...
def well_field():
    '''Harmonic electrostatic well plus the axial magnetic field.

    E is written analytically rather than differenced from the
    potential, so it is curl-free to round-off.
    '''

    def E_func(x):
        return -np.array([2.0 * x[0] - 1.0, 4.0 * x[1], 6.0 * x[2]])

    def B_func(x):
        return np.array([0.0, 0.0, np.hypot(x[0], x[1])])

    return StaticField(E_func=E_func, B_func=B_func)
