...
def Cubic_E(x):
    ...
    E_out[0] = -3 * x[0]**2 * (4 * x[0] / 15 + 1)
    E_out[1] = -3 * x[1]**2 * (4 * x[1] / 3 - 1)
    E_out[2] = -4 * x[2]**3
    return E_out

# Electric potential corresponding to E-field above.
def Non_quadratic_phi(x):
    phi_out = x[0]**3 *(x[0] / 5 + 1) + x[1]**3 * (x[1] - 1) + x[2]**4
    return phi_out
...
experiment_root_folder = 'experiments/non_quadratic_electric_potential/'
...
for integrator in integrators:
    ...
    for num_steps in step_array:
        ...
        if integrator == DiscreteGradient:
            keyword_args = {'phi': Non_quadratic_phi}
        else:
            keyword_args = {}
